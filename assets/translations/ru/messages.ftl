...
msg-extra-main =
    🗑 <b>Удаление одного устройства</b>
    Позволяет пользователю удалить конкретное устройство из списка.
    Статус: { $device_single_enabled ->
        [1] ✅ Включено
       *[0] ❌ Выключено
    } | ⏱ КД: { $device_single_cooldown ->
        [0] нет
        *[other] { $device_single_cooldown }ч
    }

    🔄 <b>Удаление всех устройств</b>
    Позволяет пользователям удалить все устройства одним нажатием.
    Статус: { $device_all_enabled ->
        [1] ✅ Включено
       *[0] ❌ Выключено
    } | ⏱ КД: { $device_all_cooldown ->
        [0] нет
        *[other] { $device_all_cooldown }ч
    }

    🔗 <b>Сброс ссылки подписки</b>
    Позволяет переиздать ссылку подключения (инвалидирует старую).
    Статус: { $link_reset_enabled ->
        [1] ✅ Включено
       *[0] ❌ Выключено
    } | ⏱ КД: { $link_reset_cooldown ->
        [0] нет
        *[other] { $link_reset_cooldown }ч
    }

    👤 <b>Сброс реф. ссылки</b>
    Позволяет пользователям изменить свою реферальную ссылку.
    Статус: { $referral_reset_enabled ->
        [1] ✅ Включено
       *[0] ❌ Выключено
    } | ⏱ КД: { $referral_reset_cooldown ->
        [0] нет
        *[other] { $referral_reset_cooldown }ч
    }

msg-extra-set-cd = ⏱ Введите cooldown в часах (0 — без ограничений, макс. 8760):

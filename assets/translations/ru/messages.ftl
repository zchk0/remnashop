...
msg-broadcast-custom-button =
    ...
    • <b>Включена</b>: { $custom_button_enabled ->
        [1] ✅
        *[0] ❌
    }
    </blockquote>

    Включить кнопку в рассылку можно переключателем в общем списке кнопок.
...
msg-broadcast-excluded-users =
    <b>🚫 Исключения из рассылки</b>

    <blockquote>
    • <b>Указано Telegram ID</b>: { $excluded_users_count }
    • <b>ID</b>: <code>{ $excluded_user_ids }</code>
    • <b>По времени регистрации</b>: { $registration_exclusion_days ->
        [7] исключаются аккаунты старше 7 дней
        [30] исключаются аккаунты старше 30 дней
        [90] исключаются аккаунты старше 90 дней
       *[0] не исключаются
    }
    • <b>Получателей после исключений</b>: { $audience_count }
    </blockquote>

    Выберите предельный возраст аккаунта кнопкой ниже. Например, неделя исключит пользователей, зарегистрированных более 7 дней назад. Квартал считается как 90 дней.

    Чтобы задать список, отправьте Telegram ID пользователей через пробел, запятую, точку с запятой или с новой строки. Дубликаты будут удалены автоматически. Можно указать до 1000 ID.
